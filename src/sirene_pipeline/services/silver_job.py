...
def run_silver_transformation(dataset_name: str) -> None:
    ...
    naf_col = "activitePrincipaleEtablissement" if "activitePrincipaleEtablissement" in new_df.columns else "activitePrincipaleUniteLegale"
    if naf_col in new_df.columns:
        new_df["activity_sector"] = new_df[naf_col].str[:2]

    date_creation_col = "dateCreationEtablissement" if "dateCreationEtablissement" in new_df.columns else "dateCreationUniteLegale"
    ...
    if silver_output.exists():
        ...
        final_df = pd.concat([existing_df, new_df]).drop_duplicates(
            subset=['siret'] if 'siret' in new_df.columns else ['siren'], 
            keep='last'
        )
    else:
        final_df = new_df
